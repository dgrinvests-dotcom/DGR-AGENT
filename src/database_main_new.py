...
async def process_incoming_sms(from_number: str, message: str, to_number: str):
    ...
    try:
        ...
        # Run the conversation through LangGraph with proper inbound handling
        print(f"🤖 Processing through AI system...")
        
        # Set the conversation mode to inbound response
        ...
        # Check if AI response was generated and send via SMS
        if result.get("messages") and len(result["messages"]) > 1:
            ...
            print(f"⚠️ No AI response messages found - checking for generated response")
            
            # Check if there's a generated response in the result
            if result.get("generated_response"):
                print(f"📤 Sending generated response: {result['generated_response'][:100]}...")
                ...
                print(f"⚠️ No response generated by LangGraph system")
